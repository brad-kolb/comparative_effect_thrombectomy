...
The pooled observed odds ratio for all the trials was `r comma(pooled_or)`. The pooled observed odds ratios by category are shown in the following table.
...
data %>% 
  ...
  summarise(r_c = sum(r_c),
            ...
            n_t = sum(n_t)) %>% 
  mutate(Category = 
           c("Large", "Small", "Late", "Basilar"),
         OR = exp(log(r_t / (n_t - r_t)) - log(r_c / (n_c - r_c))),
         SE = sqrt(1/r_t + 1/(n_t - r_t) + 1/r_c + 1/(n_c - r_c))
  ) %>% 
  select(Category, OR) %>% 
  add_row(Category = "POOLED", OR = pooled_or)
```
