...
dfs <- list(
  ...
  "large" = data %>% filter(K == 1) %>% make_data_frame(),
  "early" = data %>% filter(K == 2) %>% make_data_frame(),
  "late" = data %>% filter(K == 3) %>% make_data_frame(),
  "basilar" = data %>% filter(K == 4) %>% make_data_frame()
)
...
saveRDS(model_fits, file = here("fits", "binomial_fits.RDS"))
...
saveRDS(pps, file = here("fits", "binomial_pps.RDS"))

# check outputs ----
readRDS(here("fits", "binomial_fits.RDS")) %>% 
  str()
readRDS(here("fits", "binomial_pps.RDS")) %>% 
  str()
...
summary_pps_skeptical <- summarize_fit(
  pps$skeptical, "pps", "binomial", "skeptical")
summary_pps_diffuse <- summarize_fit(
  pps$diffuse, "pps", "binomial", "diffuse")
...
binomial_summaries <- bind_rows(summary_pps_skeptical,
                             summary_pps_diffuse,
                             lapply(summaries, bind_rows))

saveRDS(binomial_summaries, file = here("fits", "binomial_summaries.RDS"))
...
summary <- readRDS(here("fits", "binomial_summaries.RDS")) 
...
p1 <- plot_or(summary, "binomial", "early", trials_early)
p2 <- plot_or(summary, "binomial", "large", trials_large)
p3 <- plot_or(summary, "binomial", "late", trials_late)
p4 <- plot_or(summary, "binomial", "basilar", trials_basilar)
...
ggsave(here("plots", "forest_binomial.png"), 
