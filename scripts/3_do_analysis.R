...
plot(df_rd$median, as.numeric(df_rd$names), xlim = c(0, max(50)), 
     ...
     xlab = "Pr(independent) without treatment",
     pch = 19,
     col = 1, main = "Figure 1")
...
plot(df_rd$median, 1:4, xlim = c(0, max(35)), 
     ylim = c(1 - extra_space, 4 + extra_space),
     yaxt = "n", xaxt = "n", ylab = NA, 
     xlab = expression(Delta~"Pr(independent) with treatment"),
     pch = 19,
     col = 1, main = "Figure 1")
...
df1 <- data.frame(name = "large", nnt = c("<100", "<10", "<5"), value = c(x_100, x_10, x_5))
...
x_10 <- format(length(x[x >.1])/length(x)*100, digits = 2)
x_5 <- format(length(x[x >.2])/length(x)*100, digits = 2)
df2 <- data.frame(name = "small", nnt = c("<100", "<10", "<5"), value = c(x_100, x_10, x_5))
...
x_10 <- format(length(x[x >.1])/length(x)*100, digits = 2)
x_5 <- format(length(x[x >.2])/length(x)*100, digits = 2)
df3 <- data.frame(name = "late", nnt = c("<100", "<10", "<5"), value = c(x_100, x_10, x_5))
...
x_10 <- format(length(x[x >.1])/length(x)*100, digits = 2)
x_5 <- format(length(x[x >.2])/length(x)*100, digits = 2)
df4 <- data.frame(name = "basilar", nnt = c("<100", "<10", "<5"), value = c(x_100, x_10, x_5))
...
plot(NA, NA, type = "n", xlim = c(1, 3), ylim = c(0, 100),
     xlab = "NNT in a new trial", ylab = "Probability", main = "Figure 4",
     xaxt = "n", yaxt = "n")
...
pdf(here("plots", "five.pdf"), width = 10, height = 3)

# layout 
par(cex.main = 1, mar = c(4, 2, 4, 2) + 0.1, mgp = c(3.5, 1, 0), cex.lab = 1, 
