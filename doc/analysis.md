...
treatment?’ and ‘how much more likely is a meaningful improvement with
this treatment?’
...
basilar strokes, and 9% (95% PI 2.0% - 21.6%) for large strokes (figure
1).
...
98% for small core, 98% with late window, and 98% with basilar
occlusion. A 1% effect size corresponds to a number needed to treat of
100.
...
for small stroke, 26% (95% PI 13.2% - 44.9%) for late stroke, 20% (95%
PI 10.0 - 36.0%) for basilar stroke, and 8% (95% PI 3.9% - 17.5%) for
large stroke (figure 3).
...
changes those chances in absolute terms, avoiding reference to relative
changes. Traditional meta-analyses, while statistically rigorous, often
fail to provide information in this patient-centered format.
...
This gets to the heart of our approach: we use prediction not to
forecast specific outcomes, but to understand uncertainty. By focusing
on hypothetical future trials, we create a rigorous baseline for
discussing how individual patient characteristics might alter
...
face: What range of outcomes might we expect? How confident are we in
meaningful improvement?

### Clinical importance over statistical significance

This approach reveals important clinical insights. Despite a
statistically significant predicted effect in future trials regardless
of stroke type, the probability that the predicted effect is clinically
...
chance of this effect drops to just 7%. These probability statements
provide a more nuanced framework for discussing treatment decisions than
simply saying the treatment is ‘proven effective’ - they help clinicians
communicate both the promise and limitations of thrombectomy across
different stroke types.

For example, when counseling a patient with a large stroke, rather than
just noting that thrombectomy is ‘evidence-based’, a clinician can
explain that while some improvement is very likely, the chance of
dramatic benefit is small. This more complete picture helps set
appropriate expectations while still supporting the treatment’s role.

### Absolute over relative effects

This distinction between statistical and clinical significance becomes
even clearer when we focus on absolute rather than relative improvements
(figure 5). For large strokes, saying ‘thrombectomy doubles your chances
of a good outcome’ sounds impressive, and is supported by our analysis
if we focus only on relative effect size, but masks the underlying
clinical reality: moving from an 8% to a 17% chance of functional
...
for small strokes, a ‘57% relative increase’ in good outcomes translates
more meaningfully for patients as moving from a 30% to a 49% chance of
functional independence.

Our emphasis on absolute effects reflects a broader commitment to
transparent risk communication. While relative measures can make
...
underlying mathematical complexity rather than hiding it behind
